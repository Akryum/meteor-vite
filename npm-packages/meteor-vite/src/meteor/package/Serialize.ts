...
import type { ModuleExport, PackageScopeExports } from './Parser'
...
export default new class Serialize {
  public moduleExport(module: ModuleExport, packageId: string) {
    if (module.type === 're-export') {
      const from = module.from?.startsWith('.')
  ...
  public parseModules({ packageId, packageScope, modules }: { packageId: string; packageScope: PackageScopeExports; modules: ModuleExport[] }) {
    interface PlacementGroup { top: string[]; bottom: string[] }
    const result: {
    ...
    modules.forEach((moduleExport) => {
      if (!moduleExport.name)
        return
      if (moduleExport.type === 'global-binding')
        return
